...
void CreateMatrix()
{
   int index = 0; //: индекс узла
   double hx1, hx2, hy1, hy2, ubeta; //: h-ки приращения аргументов
   // Нижняя линия области "Г"
   f[0] = U(X[0], Y[0]); // Левый нижний угол
   index++;
   hy1 = abs(Y[0] - Y[1]);
   ...
   f[index] = U(X[gX - 1], Y[0]); // Правый нижний угол
   index++;
   ...
   for (int i = gY; i < nY - 1; i++) {
      ...
      if (boundary[4] == 3) { // Правое краевое
